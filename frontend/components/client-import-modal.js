  ...
  return (
    <div className={`alma-backdrop alma-anim-${phase}`} onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 65, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "rgba(58,47,38,0.4)" }}>
      <div onClick={(event) => event.stopPropagation()} className={`alma-card alma-modal alma-anim-${phase}`} style={{ width: "min(760px, 100%)", maxHeight: "90vh", overflow: "auto", borderRadius: 18, padding: 26, position: "relative", background: "#FDFCFA" }}>
        ...
        <p style={{ color: "#8C6E50", fontSize: 13, margin: "0 0 18px", lineHeight: 1.45 }}>Primero revisaremos el archivo. La ficha identifica a cada clienta; se pueden importar fichas sin celular. La importación solo crea fichas nuevas o completa campos vacíos; nunca reemplaza datos ya registrados.</p>
        ...
        {preview && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 9, marginTop: 18 }}>
              {[["Listas para importar", preview.validRows], ["Fichas completas", preview.completeRows], ["Con datos por completar", preview.incompleteRows], ["Bloqueadas", preview.invalidRows], ["Con observaciones", preview.warningRows]].map(([label, value]) => (
            ...
            <p style={{ fontSize: 12, color: "#8C6E50", margin: "14px 0 8px" }}>Encabezados detectados en la fila {preview.headerRowNumber}. Columnas reconocidas: {preview.recognized.join(", ") || "ninguna"}</p>
